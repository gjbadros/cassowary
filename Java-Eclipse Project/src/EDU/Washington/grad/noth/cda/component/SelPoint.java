...
public class SelPoint implements Cloneable {
    ...
    protected Vector<ConstrComponent> interCC;
    // Which constraint objs. have an interest in this SelPoint?
    protected Vector<Constraint> interConstr;
    ...
    public SelPoint(ClSimplexSolver solver, int x, int y, boolean isSelected,
            boolean isHighlighted, int r, int b) {
        this.solver = solver;
        this.x = x;
        this.y = y;
        ...
        interCC = new Vector<ConstrComponent>(4);
        interConstr = new Vector<Constraint>(4);
        owner = null;
        ...
        try {
            ...
            leftBorderConstraint = new ClLinearInequality(clX, CL.Op.GEQ, 3.0);
            topBorderConstraint = new ClLinearInequality(clY, CL.Op.GEQ, 3.0);
    ...
    public void setBorderConstraints(int r, int b) {
        ...
        try {
            rightBorderConstraint = new ClLinearInequality(clX, CL.Op.LEQ,
                    (double) r);
            bottomBorderConstraint = new ClLinearInequality(clY, CL.Op.LEQ,
                    (double) b);
    ...
    public boolean inside(Point sp) {
        if (this.inside(new Rectangle(sp.x - CDA_G.SELBOXSIZE / 2, sp.y
                - CDA_G.SELBOXSIZE / 2, CDA_G.SELBOXSIZE, CDA_G.SELBOXSIZE)))
            return true;
        else
            return false;
    }
    ...
    public void draw(Graphics g) {
        ...
        if (isHighlighted && !isSelected) {
            ...
            if (isSelected) {
                ...
                g.drawRect(x - CDA_G.DRAWBOXSIZE / 2,
                        y - CDA_G.DRAWBOXSIZE / 2, CDA_G.DRAWBOXSIZE,
                        CDA_G.DRAWBOXSIZE);
            }
        }
    }
    ...
    public Object clone() {
        ...
        s.owner = this.owner;
        s.interCC = (Vector<ConstrComponent>) this.interCC.clone();
        s.interConstr = (Vector<Constraint>) this.interConstr.clone();
    ...
    @SuppressWarnings("unchecked")
    public Vector<ConstrComponent> getInterestedCC() {
        return (Vector<ConstrComponent>) interCC.clone();
    }
    ...
    @SuppressWarnings("unchecked")
    public Vector<Constraint> getInterestedConstr() {
        return (Vector<Constraint>) interConstr.clone();
    }
...
/*
 * $Log$
 * Revision 1.1  2008/08/12 21:31:41  larrymelia
 ...
 * 
 * Revision 1.3 1998/06/23 02:08:41 gjb Added import of cassowary package so
 * that the cda doesn't need to be in the same package as the solver.
 * 
 * Revision 1.2 1998/05/09 00:30:22 gjb Remove cr-s
 * 
 * Revision 1.1 1998/05/09 00:10:57 gjb Added
 * 
 * Revision 1.19 1998/04/23 09:00:32 Michael Removed printing msg and changed
 * clone behavior
 * 
 * Revision 1.18 1998/04/20 09:53:07 Michael Removed debugging printing
 * 
 * Revision 1.17 1998/04/19 04:09:28 Michael Added code for SP removal
 * notification
 * 
 * Revision 1.16 1998/04/15 00:08:37 Michael Changed drawing of highlighted
 * points to be hollow green box
 * 
 * Revision 1.15 1998/04/08 05:40:42 Michael Commented out printing msgs
 * 
 * Revision 1.14 1998/04/01 10:14:33 Michael Added data/functions for interested
 * CC/Constraint obj lists Added unique ID field Changed many data members to
 * protected instead of public access
 * 
 * Revision 1.13 1998/03/25 02:46:20 Michael Updated to use CDA_G globals
 * 
 * Revision 1.12 1998/03/19 09:17:33 Michael Added additional debugging
 * printouts
 * 
 * Revision 1.11 1998/03/19 04:36:17 Michael Added finalize method
 * 
 * Revision 1.10 1998/03/02 06:43:27 Michael Added alternate draw method to
 * specify new color
 * 
 * Revision 1.9 1998/02/25 10:38:17 Michael Added clone() and copy() methods.
 * Added vars to explicitly store all constraints. Added function to remove all
 * constraints associated with a SelPoint
 * 
 * Revision 1.8 1998/02/22 04:59:48 Michael Fixed off-by-one error and made
 * point boxes larger
 * 
 * Revision 1.7 1998/02/17 08:28:24 Michael Changed visual appearance of points
 * 
 * Revision 1.6 1998/02/16 10:36:00 Michael Added hooks to assist with window
 * resizes
 * 
 * Revision 1.5 1998/02/15 11:33:06 Michael Added helper functions for edit
 * constraints
 * 
 * Revision 1.4 1998/02/15 07:13:38 Michael Added Cassowary solver hooks
 * 
 * Revision 1.3 1998/02/09 02:26:23 Michael Fixed bug in drawing of selection
 * indication rectangle
 * 
 * Revision 1.2 1998/02/06 08:36:28 Michael Fixed compile errors
 */
