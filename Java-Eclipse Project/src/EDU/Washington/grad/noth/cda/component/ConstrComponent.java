...
public abstract class ConstrComponent {
    ...
    public Vector<SelPoint> selPoints;
    ...
    public ConstrComponent(ClSimplexSolver solver) {
        this.solver = solver;
        selPoints = new Vector<SelPoint>(5);
        bbox = new Rectangle();
        isHighlighted = false;
        isSelected = false;
    ...
    public void unselect() {
        for (int a = 0; a < selPoints.size(); a++) {
            selPoints.elementAt(a).clearSelected();
        }
        updateIsSelected();
    }
    ...
    public void highlight(Point p, boolean isShiftDown) {
        ...
        for (a = 0; a < selPoints.size(); a++) {
            selPoints.elementAt(a).isHighlighted = false;
        }
    ...
    public final void replaceSelPoint(SelPoint origsp, SelPoint newsp) {
        ...
        for (a = 0; a < selPoints.size(); a++) {
            ...
            if (sp == origsp) { // Compare pointers
                // Allow notification to subclass that a point has been
                // replaced,
                // so it can establish any constraints on it that are subclass-
                // specific (IE mark new pt as midpoint of line, etc.)
                // When this call returns, there should be no constraints
                // associated
                // with the given point left in the receiver; they should have
                // been
         ...
         * selPoints); System.out.println("CC.repSP: After, bbox = " + bbox);
         */
    }
 ...
 * 
 * Revision 1.3 1998/06/23 02:08:37 gjb Added import of cassowary package so
 * that the cda doesn't need to be in the same package as the solver.
 * 
 * Revision 1.2 1998/05/09 00:30:19 gjb Remove cr-s
 * 
 * Revision 1.1 1998/05/09 00:10:55 gjb Added
 * 
 * Revision 1.17 1998/04/20 09:53:31 Michael Removed debugging msg
 * 
 * Revision 1.16 1998/04/19 04:09:49 Michael Increased thickness of blue
 * selection box
 * 
 * Revision 1.15 1998/04/15 00:09:29 Michael Added function to accumulate vector
 * of SP's at a point
 * 
 * Revision 1.14 1998/04/10 01:59:11 Michael Added getEndSP method
 * 
 * Revision 1.13 1998/04/08 05:44:57 Michael Commented out printing stmts and
 * changed interface to highlight
 * 
 * Revision 1.12 1998/04/02 07:01:28 Michael Added bounding box change
 * notification
 * 
 * Revision 1.11 1998/04/01 10:13:19 Michael Added data and functions for
 * interested Constraint obj back-ptrs
 * 
 * Revision 1.10 1998/03/27 06:27:38 Michael Added helper function to update
 * entire CC selection status
 * 
 * Revision 1.9 1998/03/25 02:46:39 Michael Added bounding box functions
 * 
 * Revision 1.8 1998/03/19 09:16:06 Michael Fixed logic bug where constraints on
 * a shared SelPoint may be removed
 * 
 * Revision 1.7 1998/02/25 10:40:01 Michael Added functions for replacing a
 * SelPoint (used for sharing)
 * 
 * Revision 1.6 1998/02/17 08:27:17 Michael Added method to return set of all
 * selected point
 * 
 * Revision 1.5 1998/02/16 10:34:19 Michael Added method to help with changing
 * constraints on window resize
 * 
 * Revision 1.4 1998/02/15 11:34:04 Michael Modified internals to facilitate
 * integration with Cassowary
 * 
 * Revision 1.3 1998/02/15 07:12:42 Michael Added highlight method and moved to
 * access function model for selection
 * 
 * Revision 1.2 1998/02/06 08:36:40 Michael Fixed compile errors
 */
