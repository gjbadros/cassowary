...
public class ColocationConstraint extends Constraint {
    ...
    public String toString() {
        if (selPointList.size() < 1) {
            return new String("ColocationConstr at [], interestedCC = "
                    + ccList);
        } else {
            SelPoint sp = (SelPoint) selPointList.elementAt(0);
            String s = new String("ColocationConstr at " + sp
                    + ", interestedCC = " + ccList);
            return s;
        }
    }
    ...
    public void notifySPRemoval(SelPoint sp) {
        if (selPointList.contains(sp)) {
            /*
             * System.out.println("ColocConstr.notSPRem: Removing " + sp +
             * " from " + selPointList);
             */
 ...
 * 
 * Revision 1.3 1998/06/23 02:08:48 gjb Added import of cassowary package so
 ...
 * 
 * Revision 1.7 1998/04/20 09:52:44 Michael Added code for
 * selection/highlighting
 * 
 * Revision 1.6 1998/04/19 04:11:17 Michael Updated canDiscard conditions
 * 
 * Revision 1.5 1998/04/08 23:39:46 Michael Changes so a colocation constraint
 * is discardable when only one CC cares about it.
 * 
 * Revision 1.4 1998/04/01 10:20:36 Michael Added methods for new functionality
 * 
 * Revision 1.3 1998/03/27 06:29:26 Michael Changed Coloc. constraint to be
 * placeholder for icon; constraints are in group
 */
