    ...
    public void addConstraints() {
        ...
        if (relConstrs.size() != targetCC.selPoints.size()) {
            ...
            for (a = 0; a < targetCC.selPoints.size(); a++) {
                ...
                try {
                    cli = new ClLinearInequality(sp.Y(), CL.Op.LEQ, srcCC.topSP
                            .Y());
                } catch (ExCLInternalError e) {
                    System.out
                            .println("AboveConstr.constructor: ExCLInternalError on #"
                                    + a);
                    return;
                }
        ...
        for (a = 0; a < relConstrs.size(); a++) {
            ...
            try {
                ...
                System.out
                        .println("AboveConstr.addConstr: ExCLInternalError adding #"
                                + a + " = " + cli);
            } catch (ExCLRequiredFailure e) {
                System.out
                        .println("AboveConstr.addConstr: ExCLRequiredFailure "
                                + "adding #" + a + " = " + cli);
            }
        }
 ...
 * 
 * Revision 1.3 1998/06/23 02:08:43 gjb Added import of cassowary package so
