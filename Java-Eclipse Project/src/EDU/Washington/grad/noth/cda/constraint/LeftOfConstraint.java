    ...
    public void addConstraints() {
        ...
        if (relConstrs.size() != targetCC.selPoints.size()) {
            ...
            for (a = 0; a < targetCC.selPoints.size(); a++) {
                ...
                try {
                    cli = new ClLinearInequality(sp.X(), CL.Op.LEQ,
                            srcCC.leftSP.X());
                } catch (ExCLInternalError e) {
                    System.out
                            .println("LeftOfConstr.constructor: ExCLInternalError on #"
                                    + a);
                    return;
                }
        ...
        for (a = 0; a < relConstrs.size(); a++) {
            ...
            try {
                ...
                System.out
                        .println("LeftOfConstr.addConstr: ExCLInternalError adding #"
                                + a + " = " + cli);
            } catch (ExCLRequiredFailure e) {
                System.out
                        .println("LeftOfConstr.addConstr: ExCLRequiredFailure "
                                + "adding #" + a + " = " + cli);
            }
        }
 ...
 * 
 * Revision 1.3 1998/06/23 02:08:49 gjb Added import of cassowary package so
