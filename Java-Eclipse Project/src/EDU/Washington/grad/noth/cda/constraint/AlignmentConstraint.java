...
import java.util.Vector; // import CDA_G;
...
public class AlignmentConstraint extends Constraint {
    ...
    // the CC's
    protected Vector<ClLinearEquation> relConstrs;
    ...
    public AlignmentConstraint(ClSimplexSolver solver,
            Vector<ConstrComponent> ccVector, int alignment) {
        ...
        /*
         * // The constraint also needs to be interested in every SelPoint of
         * the // target, in case one changes. It doesnot need to be explicitly
         * // concerned about those in the src, as any changes to them will
         * alter // its bounding box. for ( a = 0; a <
         * targetCC.selPoints.size(); a++ ) { sp = (SelPoint)
         * targetCC.selPoints.elementAt(a); addSelPoint(sp);
         * sp.addInterestedConstr(this); } srcCC.addInterestedConstr(this);
         * targetCC.addInterestedConstr(this);
         */

        targetSP = null;
        relConstrs = new Vector<ClLinearEquation>(1);
        addConstraints();
    }
    ...
    public void removeConstraints() {
        ...
        for (a = 0; a < relConstrs.size(); a++) {
            ...
            try {
                ...
                System.out
                        .println("AlignConstr.remConstr: ExCLConstraintNotFound "
                                + "removing #" + a + " = " + cle);
            }
        }
    ...
    public void replaceSelPoint(SelPoint oldsp, SelPoint newsp) {
         ...
         * 
         * if ( ccList.size() != 2 ) { System.out.println("AlignConstr.repSP:
         * Ill-formed AlignConstraint!"); return; } else { srcCC =
         * (ConstrComponent) ccList.elementAt(0); targetCC = (ConstrComponent)
         * ccList.elementAt(1); }
         * 
         * if ( srcCC.selPoints.contains(oldsp) ) { // Doesn't matter...updating
         * the bounding box of the src CC affects // the change. return; }
         * 
         * if ( targetCC.selPoints.contains(oldsp) ) { // Remove constraint on
         * that SP oldIdx = targetCC.selPoints.indexOf(oldsp); cle =
         * (ClLinearEquation) relConstrs.elementAt(oldIdx); try { if ( cle !=
         * null ) solver.removeConstraint(cle); } catch (ExCLInternalError e) {
         * System.out.println("AlignConstr.repSP: ExCLInternalError " +
         * "removing #" + oldIdx + " = " + cle); } catch (ExCLConstraintNotFound
         * e) { System.out.println("AlignConstr.repSP: ExCLConstraintNotFound "
         * + "removing #" + oldIdx + " = " + cle); } // Create a new constraint
         * to take its place try { cli = new ClLinearInequality(newsp.X(),
         * CL.LEQ, srcCC.topSP.Y()); } catch (ExCLInternalError e) {
         * System.out.println("AlignConstr.repSP: ExCLInternalError replacing #"
         * + oldIdx); } relConstrs.setElementAt(cli, oldIdx); try { if ( cli !=
         * null ) solver.addConstraint(cli); } catch (ExCLInternalError e) {
         * System.out.println("AlignConstr.repSP: ExCLInternalError adding #" +
         * oldIdx + " = " + cli); } catch (ExCLRequiredFailure e) {
    ...
    public void addConstraints() {
        ...
        if (relConstrs.size() != ccList.size()) {
            ...
            if (relConstrs.size() != 0) {
                System.out.println("AlignConstr.addConstr: relConstrs = "
            ...
            for (a = 0; a < ccList.size(); a++) {
                ...
                case BOTTOM_ALIGN:
                    ...
                    break;
                default:
                    System.out.println("AlignConstr.addConstr: Unknown type "
                            + align);
                    return;
                }
            }
            ...
            for (a = 0; a < ccList.size(); a++) {
                ...
                case LEFT_ALIGN:
                    ...
                    try {
                        ...
                        System.out
                                .println("AlignConstr.addCon: ExCLRequiredFailure!");
                    } catch (ExCLInternalError e) {
                        System.out
                                .println("AlignConstr.addCon: ExCLInternalError!");
                    }
                    break;
                case RIGHT_ALIGN:
                    sp = cc.rightSP;
    ...
    public String toString() {
        StringBuffer sb = new StringBuffer("AlignConstr: targetSP = "
                + targetSP);
        sb.append("\nccList = ");
...
/*
 * $Log$
 * Revision 1.1  2008/08/12 21:31:37  larrymelia
 ...
 * 
 * Revision 1.3 1998/06/23 02:08:45 gjb Added import of cassowary package so
 ...
 * 
 * Revision 1.3 1998/04/26 06:19:29 Michael Changed how highlighting is drawn
 * 
 * Revision 1.2 1998/04/23 08:59:30 Michael Removed printing msgs
 * 
 * Revision 1.1 1998/04/23 08:47:37 Michael Initial check-in
 */
