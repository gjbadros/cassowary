    ...
    public void addConstraints() {
        ...
        if (relConstrs.size() != targetCC.selPoints.size()) {
            ...
            for (a = 0; a < targetCC.selPoints.size(); a++) {
                ...
                try {
                    cli = new ClLinearInequality(sp.Y(), CL.Op.GEQ,
                            srcCC.bottomSP.Y());
                } catch (ExCLInternalError e) {
                    System.out
                            .println("BelowConstr.constructor: ExCLInternalError on #"
                                    + a);
                    return;
                }
        ...
        for (a = 0; a < relConstrs.size(); a++) {
            ...
            try {
                ...
                System.out
                        .println("BelowConstr.addConstr: ExCLInternalError adding #"
                                + a + " = " + cli);
            } catch (ExCLRequiredFailure e) {
                System.out
                        .println("BelowConstr.addConstr: ExCLRequiredFailure "
                                + "adding #" + a + " = " + cli);
            }
        }
 ...
 * 
 * Revision 1.3 1998/06/23 02:08:47 gjb Added import of cassowary package so
