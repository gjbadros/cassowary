...
import EDU.Washington.grad.noth.cda.constraint.*;
...
class DrawPanel extends Panel {
    ...
    Vector<ConstrComponent> curCC;

    // List of active Constraint objects
    Vector<Constraint> curConstr;
    ...
    DrawPanel() {
        ...
        curCC = new Vector<ConstrComponent>(10);

        curConstr = new Vector<Constraint>(10);
    ...
    public void setDesiredSize(int dW, int dH) {
         ...
         * d.height + ")"); System.out.println("DP.setDS: Parent size = " +
         * getParent().getSize());
         */
        ...
        // Discard the off-screen buffer; the next repaint will generate a new
        // one
        paintBuffer = null;
    ...
    public Vector<ConstrComponent> getAllSelectedCC() {

        int a;
        Vector<ConstrComponent> v = new Vector<ConstrComponent>(4);
        ConstrComponent c;
    ...
    // the isSelected flag)
    public Vector<Constraint> getAllSelectedConstr() {
        int a;
        Vector<Constraint> retv = new Vector<Constraint>(4);
        Constraint c;

        for (a = 0; a < curConstr.size(); a++) {
            c = curConstr.elementAt(a);
            if (c.getisSelected())
                retv.addElement(c);
        }
    ...
    public Vector<SelPoint> getAllSelectedSelPoints() {
        Vector<SelPoint> allSelPoints = new Vector<SelPoint>(10), v;
        int a, nsp;
        ConstrComponent c;
        SelPoint sp;
        ...
        for (a = 0; a < curCC.size(); a++) {
            ...
            for (nsp = 0; nsp < v.size(); nsp++) {
                ...
                /*
                 * System.out.println("DP.getAllSelSP: Considering " + sp +
                 * " to add to " + allSelPoints);
                 */
    ...
    public void createNewConstraint(String cc) {
        ...
        Vector<ConstrComponent> v;
        Vector<SelPoint> allSelPoints = null;
        SelPoint sp;
        ...
        if (cc.equals("LeftAlign")) {
            ...
            ac = new AlignmentConstraint(solver, v,
                    AlignmentConstraint.LEFT_ALIGN);
        ...
        if (cc.equals("RightAlign")) {
            ...
            Constraint ac;
            ac = new AlignmentConstraint(solver, v,
                    AlignmentConstraint.RIGHT_ALIGN);
    ...
    public void establishRelationalConstr(ConstrComponent target) {
        Vector<ConstrComponent> v = getAllSelectedCC();
        ConstrComponent c;
        Constraint rc;
        int a;
         ...
         * System.out.println("DP.estRelConstr: type = " +
         * relationalConstrType);
         */

        if (v.contains(target)) {
            /*
             * System.out.println("DP.estRelConstr: Selected CC list contains
             * target!");
             */
            stopPickingTargetCC();
            return;
    ...
    private void establishColocationConstraint(Vector<SelPoint> allSelPoints) {
        int a, b;
        Vector<ConstrComponent> v, CCofStack;
        Vector<Constraint> cv;
        SelPoint basesp, sp;
        Constraint c;
        ConstrComponent constrc;
        ...
        CCofStack = new Vector<ConstrComponent>(4);
        for (a = 0; a < allSelPoints.size(); a++) {
            sp = (SelPoint) allSelPoints.elementAt(a);
        ...
        /*
         * // Arbitrarily let the first selected SP be the base basesp =
         * (SelPoint) allSelPoints.elementAt(0);
         */

        // Scan the vector of points and pick the best one to use as the base.
        // Currently, choose any point w/ an anchor constraint
        // cv = new Vector<Constraint>(4);
        basesp = null;
        Vector<Constraint> interConstr = new Vector<Constraint>(4);
        for (a = 0; a < allSelPoints.size(); a++) {
            sp = (SelPoint) allSelPoints.elementAt(a);
            ...
            for (b = 0; b < interConstr.size(); b++) {
                ...
                if (c instanceof AnchorConstraint) {
                    if (basesp != null) {
                        System.out
                                .println("Two points to merge have anchor constraints:");
        ...
        cv = basesp.getInterestedConstr();
        for (a = 0; a < cv.size(); a++) {
            if (cv.elementAt(a) instanceof ColocationConstraint) {
                coloc = (ColocationConstraint) cv.elementAt(a);
                break;
            }
        }
        if (coloc == null) {
        ...
        // We also need to scan all constraints in case multiple colocation
        // constrs
        // got merged.
        cv = new Vector<Constraint>(4);
        /*
    ...
    public void removeConstrComponent(ConstrComponent c) {
        ...
        if (curCC.contains(c)) {
            ...
            c.setBorderConstraints(desiredWidth, desiredHeight);
            // Also notify all of c's constraints and interested components
            // about
            // its removal
        } else {
    ...
    public void displayAppInfo() {
        ...
        for (a = 0; a < curCC.size(); a++)
            System.out.println(curCC.elementAt(a));
        System.out.println(curConstr.size() + " Constraint objects:");
        for (a = 0; a < curConstr.size(); a++)
            System.out.println(curConstr.elementAt(a));
    ...
    public void displayFullInfo() {
        int a;
        Vector<SelPoint> v = getAllSelectedSelPoints();
        SelPoint sp;
    ...
    /*
     * private Vector<ConstrComponent> componentsOfSelPoint(SelPoint sp) {
    ...
    public void removeAll() {
        ...
        // Remove all CC's
        /*
         * for(a = 0; a < curCC.size(); a++) { // c = curCC.elementAt(a); //
    ...
    public void deleteCC() {
        Vector<ConstrComponent> selCC, keepCC;
        Vector<Constraint> v, selConstr, iCon;
        ...
        for (a = 0; a < selCC.size(); a++) {
            ...
            // First, notify each constraint that cares about the component that
            // the
            // CC is going away.
            ...
            for (b = 0; b < c.selPoints.size(); b++) {
                ...
                if (sp.getInterestedCC().size() == 0) {
                    ...
                    /*
                     * System.out.println("DP.delCC: Can't discard SP " +
                     * sp.fullInfo());
                     */
                }
            }
         ...
         */
        Vector<ConstrComponent> vc;

        for (a = 0; a < selConstr.size(); a++) // For each selected constraint,
        // remove it.
        {
            constr = (Constraint) selConstr.elementAt(a);

            // If the constraint that is being deleted is a colocation
            // constraint, clone the point so that every interested CC gets its
            // own
            // copy.
            if (constr instanceof ColocationConstraint) {
                ...
                for (b = 1; b < vc.size(); b++) {
                    c = (ConstrComponent) vc.elementAt(b);
                    repsp = (SelPoint) sp.clone();
                    /*
                     * // Each replacement SP should not be interested in the
                     * constraint // that is going away...
                     * repsp.removeInterestedConstr(constr);
                     */
                    // Remove all anchor constraints from a cloned pt.
                    // ...also remove interest in any CC's where the coloc.
                    // constr.
                    // was the only reason for interest
                    /*
                     * System.out.println("DP.delCC: repsp " + repsp +
                     * " interCC = " + repsp.interCC);
                     */

                    // Get all constraint objs. interested in point
                    iCon = repsp.getInterestedConstr();
                    keepCC = new Vector<ConstrComponent>(4);
                    /*
                     ...
                     */
                    // For each CC that the to-be-deleted constraint was
                    // interested in,
                    // remove repsp's interest unless it's in the keepCC list.
                    selCC = repsp.getInterestedCC();
                    for (d = 0; d < iCon.size(); d++) {
                        iC = (ConstrComponent) selCC.elementAt(d);
                        if (!keepCC.contains(iC)) {
                            /*
                             * System.out.println("DP.delCC: Removing repsp interest in "
                             * + iC);
                             */
                            ...
                            System.out
                                    .println("DP.delCC: Keeping repsp interest in "
                                            + iC);
            ...
            Vector<SelPoint> sv = constr.selPointList;
            for (b = 0; b < sv.size(); b++) {
                sp = (SelPoint) sv.elementAt(b);
                sp.removeInterestedConstr(constr);
            }
            constr.selPointList.removeAllElements();
            selCC = constr.ccList;
            for (b = 0; b < selCC.size(); b++) {
                c = (ConstrComponent) selCC.elementAt(b);
                c.removeInterestedConstr(constr);
            }
            ...
            if (constr.canDiscard()) {
                ...
                System.out.println("DP.delCC: Trying to delete constr "
                        + constr + " but canDiscard() = false!");
            }
        }
    ...
    public void getSelPointsAt(Point p, Vector<SelPoint> retv) {
        int a;
        ConstrComponent c;
    ...
    public Vector<ConstrComponent> getCCAtPoint(Point p) {
        Vector<ConstrComponent> retv = new Vector<ConstrComponent>(4);
        int a;
        ConstrComponent c;
    ...
    public Vector<Constraint> getConstrAtPoint(Point p, boolean isShiftDown) {
        Vector<Constraint> retv = new Vector<Constraint>(4);
        int a;
        Constraint c;
    ...
    public Vector<ConstrComponent> selectCC(Point p,
            boolean performMultipleSelect) {
        int a;
        ConstrComponent c;
    ...
    public void mousePressed(MouseEvent e) {
        ...
        /*
         * System.out.println("DP.mousePressed invoked, mouse at (" + e.getX() +
         * ", " + e.getY() + ")");
         */
        ...
        Vector<SelPoint> SPatPoint = new Vector<SelPoint>(4);
        ...
        if (SPatPoint.size() < 1)
            ...
            sp = (SelPoint) SPatPoint.elementAt(spVectorOffset
                    % (SPatPoint.size()));
            spVectorOffset++;
        }
        ...
        if (sp != null) {
            if (!sp.getSelected()) {
                ...
                // Make sure to reset the mouse start position in case the SP
                // hit
                // is then dragged.
                mouseStart(new Point(sp.x, sp.y));
            }
            ...
            Vector<Constraint> vc = getConstrAtPoint(e.getPoint(), e
                    .isShiftDown());
            /*
            ...
            if (vc.size() > 0) {
                if (e.isControlDown()) {
                    constr = (Constraint) vc.elementAt(0);
                    constr.setisSelected(true);
                } else {
                    unselectAll();
                    for (a = 0; a < vc.size(); a++) {
                        constr = (Constraint) vc.elementAt(a);
                        constr.setisSelected(true);
                    }
                }
            } else {
                ...
                if ((v.size() == 0) || !e.isShiftDown()) {
                    // Hitting nothing results in setting the start of a
                    // rubber-band sel.
                    // box drag. Also may need to clear all currently selected
                    // points.
                    if (!e.isControlDown()) {
                    ...
                    // There are CC's in v enclosing the given point, so select
                    // them
                    if (e.isControlDown()) {
                        c = (ConstrComponent) v.elementAt(0);
                        c.setisSelected(true);
    ...
    public void mouseReleased(MouseEvent e) {
        /*
         * System.out.println("DP.mouseReleased invoked, mouse at (" + e.getX()
         * + ", " + e.getY() + ")");
         */
        ...
        Vector<SelPoint> allSelSP = getAllSelectedSelPoints();
        Vector<ConstrComponent> movedCC, spCC;
        SelPoint sp, stSP;

        movedCC = new Vector<ConstrComponent>(4);
        ...
        if (drawingSelectionBox) {
            ...
            Vector<SelPoint> v, allSP, stackedSP;
            ConstrComponent cc;

            allSP = new Vector<SelPoint>(20);
            for (a = 0; a < curCC.size(); a++) {
                cc = curCC.elementAt(a);
            ...
            while (allSP.size() > 0) {
                ...
                v = new Vector<SelPoint>(4);
                getSelPointsAt(new Point(sp.x, sp.y), v);
                stackedSP = new Vector<SelPoint>(4);
                for (b = 0; b < v.size(); b++) {
                    // To be merged, the point must exactly share x, y coords
                    // with
                    // SelPoint sp
                ...
                if (stackedSP.size() > 1) {
                    if (stackHasActiveSP) {
                        ...
                        /*
                         * System.out.println("Inactive stack (" + sp + "): " +
                         * stackedSP);
                         */
                    }
                    for (b = 0; b < stackedSP.size(); b++)
    ...
    public void mouseDragged(MouseEvent e) {
        ...
        // Ensure that strange things don't happen when cursor is dragged
        // outside
    ...
    public boolean isValidSnapToTarget(SelPoint sp) {
        SelPoint tSP;
        Vector<SelPoint> allSelSP;
        Vector<ConstrComponent> v, activeCC;
        int a, b;
        ConstrComponent cc;
        ...
        activeCC = new Vector<ConstrComponent>(4);
        for (a = 0; a < allSelSP.size(); a++) {
    ...
    public ConstrComponent placeComponent(MouseEvent e) {
        ...
        Vector<SelPoint> v = new Vector<SelPoint>(4);
    ...
    public void resatisfyConstraints() {
        ...
        /*
         * if ( numEC == 0 ) { // Don't try a resolve with no selected points!
         * return; }
         */
 ...
 * 
 * Revision 1.4 1998/06/23 02:08:54 gjb Added import of cassowary package so
 * that the cda doesn't need to be in the same package as the solver.
 * 
 * Revision 1.3 1998/05/14 22:34:22 gjb Print full solver information upon "i"
 * keystroke
 * 
 ...
 * 
 * Revision 1.27 1998/05/05 21:42:58 Michael Now require shift key to be held
 * for constr. selection
 * 
 * Revision 1.26 1998/04/28 20:08:40 Michael Removed dead code
 * 
 * Revision 1.25 1998/04/26 06:16:12 Michael Changed layout
 * 
 * Revision 1.24 1998/04/23 09:02:31 Michael Fixed several point-merging bugs
 * w/alignment constraints Changed selection of colocation base point to favor
 * pts w/ anchor constraints Added code for alignment constraints Added
 * regeneration of border constraints on cloned SP's Relaxed agressiveness of
 * snap-to behavior Prevented colocation constraints from being formed on pts in
 * same CC
 * 
 * Revision 1.23 1998/04/20 09:55:08 Michael Changed auto-merging to only look
 * at stacked points related to a CC that was moved directly Added ability to
 * select and delete constraint objs
 * 
 * Revision 1.22 1998/04/19 04:12:47 Michael Moved establishColocationConstraint
 * to separate function Added snap-to-point for placing CC's and dragging points
 * Added automatic colocation point establishment
 * 
 * Revision 1.21 1998/04/15 00:10:18 Michael Changed selection model to allow
 * for picking only one point in a stack
 * 
 * Revision 1.20 1998/04/10 02:01:12 Michael Added ability to set size of CC
 * when placing it
 * 
 * Revision 1.19 1998/04/08 23:40:33 Michael Changes so colocation constraints
 * disappear when appropriate Dragging cursor outside of window no longer causes
 * offset problems
 * 
 * Revision 1.18 1998/04/08 05:42:56 Michael Commented out printing msgs Changed
 * selection box to be draggable in any direction (not just down-right) Changed
 * selection interface to require shift + click to select whole obj, and removed
 * highlighting of whole objs w/out shift down
 * 
 ...
 * 
 * Revision 1.15 1998/04/01 10:18:18 Michael Merged in mousePress, mouseRelease
 * functions from DrawPanelMouseMotionListener Added code to handle using and
 * maintaining back pointers
 * 
 * Revision 1.14 1998/03/27 06:24:51 Michael Added code to allow picking a
 * target CC for relational constraints
 * 
 ...
 * 
 * Revision 1.10 1998/03/02 06:45:06 Michael Extensive changes to move to
 * CCG-based list of parts
 * 
 * Revision 1.9 1998/02/25 10:40:43 Michael Added first cut of internal grouping
 * for point sharing
 * 
 ...
 * 
 * Revision 1.5 1998/02/16 10:33:40 Michael Added hooks for adding/placing a new
 * component
 * 
 * Revision 1.4 1998/02/15 11:33:38 Michael Hooked up solver to mouse movement
 * 
 * Revision 1.3 1998/02/15 07:14:35 Michael Added highlighting on mouse movement
 * Added Cassowary solver instance and hooks
 * 
 * Revision 1.2 1998/02/09 02:25:17 Michael Added mouse drag functionality Moved
 * mouse listener class inside DrawPanel class Added vector of ConstrComponents
 * stored in DrawPanel
 * 
 * Revision 1.1 1998/02/06 11:13:30 Michael Major changes in layout to handle
 * event propagation and resizing
 */
