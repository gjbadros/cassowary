...
package EDU.Washington.grad.gjb.cassowary.demos;
...
public class QuadDemo extends Applet {
    ...
    int dbDragging; // Which db is being dragged?
    // -1 for none
    ...
    public void init() {
        ...
        try {
            ...
            solver.addConstraint(
                    new ClLinearInequality(cle, CL.Op.LEQ, db[2].X()))
                    .addConstraint(
            ...
            solver.addConstraint(
                    new ClLinearInequality(cle, CL.Op.LEQ, db[1].Y()))
                    .addConstraint(
            ...
            solver.addConstraint(new ClLinearInequality(db[0].X(), CL.Op.GEQ,
                    0.0));
            solver.addConstraint(new ClLinearInequality(db[0].Y(), CL.Op.GEQ,
                    0.0));
            solver.addConstraint(new ClLinearInequality(db[1].X(), CL.Op.GEQ,
                    0.0));
            solver.addConstraint(new ClLinearInequality(db[1].Y(), CL.Op.GEQ,
                    0.0));
            solver.addConstraint(new ClLinearInequality(db[2].X(), CL.Op.GEQ,
                    0.0));
            solver.addConstraint(new ClLinearInequality(db[2].Y(), CL.Op.GEQ,
                    0.0));
            solver.addConstraint(new ClLinearInequality(db[3].X(), CL.Op.GEQ,
                    0.0));
            solver.addConstraint(new ClLinearInequality(db[3].Y(), CL.Op.GEQ,
                    0.0));

            solver.addConstraint(new ClLinearInequality(db[0].X(), CL.Op.LEQ,
                    width));
            solver.addConstraint(new ClLinearInequality(db[0].Y(), CL.Op.LEQ,
                    height));
            solver.addConstraint(new ClLinearInequality(db[1].X(), CL.Op.LEQ,
                    width));
            solver.addConstraint(new ClLinearInequality(db[1].Y(), CL.Op.LEQ,
                    height));
            solver.addConstraint(new ClLinearInequality(db[2].X(), CL.Op.LEQ,
                    width));
            solver.addConstraint(new ClLinearInequality(db[2].Y(), CL.Op.LEQ,
                    height));
            solver.addConstraint(new ClLinearInequality(db[3].X(), CL.Op.LEQ,
                    width));
            solver.addConstraint(new ClLinearInequality(db[3].Y(), CL.Op.LEQ,
                    height));
    ...
    public boolean mouseDown(Event e, int x, int y) {
        ...
        if (dbDragging != -1) {
            try {
                solver.addEditVar(db[dbDragging].X()).addEditVar(
                        db[dbDragging].Y()).beginEdit();
    ...
    public boolean mouseDrag(Event e, int x, int y) {
        if (dbDragging != -1) {
            try {
                solver.suggestValue(db[dbDragging].X(), x).suggestValue(
                        db[dbDragging].Y(), y).resolve();
            } catch (ExCLInternalError ex) {
    ...
    public void paint(Graphics g) {
        g.drawLine((int) db[0].CenterX(), (int) db[0].CenterY(), (int) db[1]
                .CenterX(), (int) db[1].CenterY());
        g.drawLine((int) db[1].CenterX(), (int) db[1].CenterY(), (int) db[2]
                .CenterX(), (int) db[2].CenterY());
        g.drawLine((int) db[2].CenterX(), (int) db[2].CenterY(), (int) db[3]
                .CenterX(), (int) db[3].CenterY());
        g.drawLine((int) db[3].CenterX(), (int) db[3].CenterY(), (int) db[0]
                .CenterX(), (int) db[0].CenterY());
        // Connecting lines
        g.drawLine((int) mp[0].CenterX(), (int) mp[0].CenterY(), (int) mp[1]
                .CenterX(), (int) mp[1].CenterY());
        g.drawLine((int) mp[1].CenterX(), (int) mp[1].CenterY(), (int) mp[2]
                .CenterX(), (int) mp[2].CenterY());
        g.drawLine((int) mp[2].CenterX(), (int) mp[2].CenterY(), (int) mp[3]
                .CenterX(), (int) mp[3].CenterY());
        g.drawLine((int) mp[3].CenterX(), (int) mp[3].CenterY(), (int) mp[0]
                .CenterX(), (int) mp[0].CenterY());
        // Control points
        for (int a = 0; a < 8; a++)
            db[a].draw(g);
        if (dbDragging != -1) {
